...
>
> **Switching between monitor layouts** (e.g. 4 physical monitors ↔ a 2-monitor RDP session) keeps every screen's layout intact. Screens of a desktop that no longer have a monitor are not squeezed onto the remaining monitors; instead they are shown as extra *pages* of that desktop, one screen per monitor, inserted right after the desktop. The number keys index pages, so with 2 monitors and desktops that had 4 screens, `alt+1` shows desktop 1 (screens 1–2), `alt+2` shows desktop 1's screens 3–4 (labelled `1/2` in status bars), `alt+3` shows desktop 2, and so on — later desktops shift accordingly. Only screens that hold windows create pages, and the screens of missing monitors are always packed onto the pages in left-to-right order, so pages never stay half-empty after an intermediate layout (e.g. the single placeholder monitor an RDP session shows while reconnecting). A screen shown on a monitor of the other orientation (portrait ↔ landscape) has its horizontal and vertical stacks swapped so the layout still fits. When the monitors come back, each screen returns to its own monitor (matched by position and size plus display device; position and size alone beat a bare device match, since RDP sessions re-assign their generic device ids on every reconnect; the slot in the layout is the last resort), the swap is undone, and the extra pages disappear. Windows that were opened on an extra page stay on extra pages and never take over a monitor's own screen. Page labels also work on the command line: `glazewm command focus --workspace 1/2`.
>
...
>
>
> Also fixed: **windows left invisible by a previous GlazeWM instance** are recovered. With the default `hide_method: 'cloak'`, neither GlazeWM's exit cleanup nor its watcher process uncloaked the windows on hidden workspaces, so after a crash (or a kill via Task Manager) those windows stayed invisible: a freshly started GlazeWM skipped them as hidden, and clicking their taskbar button focused an invisible window. Windows are now uncloaked on exit and by the watcher. If both GlazeWM and its watcher are killed at once, the next start restores the windows the previous instance recorded in `~/.glzr/glazewm/managed-windows.json` (only those, so system windows that are cloaked by design stay untouched). As a last resort, a window that receives focus while unmanaged (e.g. from its taskbar button) is managed on the spot, lifting such a stale cloak first. The same applies to windows that get ignored (via an `ignore` rule or because positioning them is denied) while on a hidden workspace: they're shown instead of staying hidden.
>
> Also fixed: **dropping a dragged window onto a neighbor could corrupt the layout** and trap GlazeWM in an endless "Non-fatal error" dialog ("No common ancestor between containers", then "Parent does not have a tiling direction" on every event). Upstream determines the drop target before it tiles the dragged window again at its old position, but that step flattens split containers that became redundant during the drag, so the drop target could be a container that no longer exists in the tree. The drop target is now resolved from the current layout, and a failed sync pass no longer repeats forever.
>
